...
export class AssetSearchConditionComponent implements OnChanges {
   ...
   */
  @ViewChild(CodeNameSelectionComponent, null) popupStatus: CodeNameSelectionComponent;
  ...
  constructor(private fb: FormBuilder, private masterService: MasterService, private sessionService: SessionService) {
  }
  ...
  changeCondition() {
    if (this.sessionInfo.currentAccessLevel === 'C' ||
      ...
      if (this.sessionInfo.currentAccessLevel === 'C') {
        ...
        this.searchScopeType1Label = '自分の管轄部署が保有（使用）部署に設定されている資産(機器)';
      }
      if (this.formSearchScopeTypeLayout.controls.searchScopeType.value === null) {
  ...
  initFormSearchCondition() {
    this.formSearchCondition = this.fb.group({
      ...
      levelAssetFlag: [null],
      contractNum: [null],
      contractNumPlural: [null],
      contractEda: [null],
      contractEdaPlural: [null],
      shisanNum: [null],
      shisanNumPlural: [null],
      relationShisanNumFlag: [false],
      //
      getApplicationId: [null],
      getApplicationIdPlural: [null],
      registApplicationId: [null],
      registApplicationIdPlural: [null],
      registDateFrom: [''],
      registDateTo: [''],
      deleteFlag: [false],
      //
   ...
   */
  dataEditChanged($event: MouseEvent) {
  ...
  private fsStateStatus(status: string) {
    switch (status) {
      ...
      case 'ASSET_DELETE':
        ...
        this.formSearchScopeTypeLayout.controls.searchScopeType.setValue(true);
        this.formSearchCondition.controls.deleteFlag.disable();
        this.comAssetSearchConditionComponent.comAllCompanyFlagEditLock = true;
        this.comAssetSearchConditionComponent.comAllCompanyFlagSelected = false;
        break;
  ...
  clearSearchCondition() {
    ...
    switch (this.currentState) {
      ...
      default:
        ...
        this.formSearchCondition.controls.astAssetTypePlural.setValue('');
        this.comAssetSearchConditionComponent.formAssetsEquipment.controls.astAssetTypePlural.setValue('');
        break;
    }
    ...
    switch (this.currentState) {
      ...
      case 'LICENSE_REPORT_ALLOC':
        ...
        this.formSearchCondition.controls.astCategory2Plural.setValue('');
        break;
      default:
        this.comAssetSearchConditionComponent.isAstCategoryEqual = true;
        this.formSearchCondition.controls.astAssetTypePlural.setValue('');
    ...
    switch (this.currentState) {
      case 'AP_CHANGE':
        if (this.getApChangeUseType() === '2') {
          console.log(this.getApChangeUseType())
          ...
          this.comAssetSearchConditionComponent.formAssetsEquipment.controls.astSystemSectionDeployFlag.setValue(true);
          this.formSearchCondition.controls.astManageTypePlural.setValue('');
        }
        break;
      default:
  ...
  getSearchCondition() {
    const resultSearchCondition = {
      searchScopeType: this.formSearchScopeTypeLayout.get('searchScopeType').value,
      assetId: this.formSearchCondition.get('assetId').value,
      assetIdPlural: this.formSearchCondition.get('assetIdPlural').value,
      levelAssetFlag: this.formSearchCondition.get('levelAssetFlag').value ? '1' : '0',
      contractNum: this.formSearchCondition.get('contractNum').value,
      contractNumPlural: this.formSearchCondition.get('contractNumPlural').value,
      contractEda: this.formSearchCondition.get('contractEda').value,
      contractEdaPlural: this.formSearchCondition.get('contractEdaPlural').value,
      shisanNum: this.formSearchCondition.get('shisanNum').value,
      shisanNumPlural: this.formSearchCondition.get('shisanNumPlural').value,
      relationShisanNumFlag: this.formSearchCondition.get('relationShisanNumFlag').value ? '1' : '0',
      getApplicationId: this.formSearchCondition.get('getApplicationId').value,
      getApplicationIdPlural: this.formSearchCondition.get('registApplicationId').value,
      registApplicationId: this.formSearchCondition.get('registApplicationId').value,
      registApplicationIdPlural: this.formSearchCondition.get('registApplicationIdPlural').value,
    ...
    const searchParams = Object.assign(resultSearchCondition, resultComSearchCondition);
    return searchParams;
  }

  private getCodeNamelist(categoryCode: string, internalCode?: string, companyCode?: string, values?) {
    this.masterService.getCodeNameList(categoryCode, internalCode, companyCode, values).subscribe((resp: CodeNameEx[]) => {
  ...
  private getApChangeUseType(): string {
    ...
    for (const iterator of this.usableCompanyList) {
      ...
      if (row.internalCode === this.sessionInfo.loginUser.companyCode && row.value4 === '1' || row.value4 === '2') {
        ret = row.value4;
        if (ret === null || ret === '') {
          ret = '0';
        }
