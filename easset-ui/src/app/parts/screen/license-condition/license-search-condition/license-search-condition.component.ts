...
export class LicenseSearchConditionComponent implements  OnChanges {
  ...
  initChangeCondition() {
    ...
    } else if (this.accessLevel === accessLevelSection.ACCESS_LEVEL_SECTION_MANAGER) {
      this.labelSearchScopeType = '自分の管轄部署が保有（使用許諾）部署に設定されているライセンス';
    }

    if ((this.accessLevel === accessLevelSection.ACCESS_LEVEL_SECTION_GENERAL || this.accessLevel === accessLevelSection.ACCESS_LEVEL_SECTION_MANAGER) && this.currentState !== 'LICENSE_ALLOC') {
  ...
  changeStateStatus(status) {
    switch (status) {
      // ライセンス照会/修正
      case statusAction.LICENSE_SEARCH:
        break;
      // ライセンス抹消
      case statusAction.LICENSE_DELETE:
      ...
      case statusAction.AP_CHANGE:
        ...
        break;
      // タームライセンス情報
      case statusAction.TERM_LICENSE:
        this.searchForm.controls.searchScopeType.disable();
        ...
        break;
      // アップグレード割当：割当元
      case statusAction.LICENSE_UPGRADE_FROM_ALLOC:
        this.searchForm.controls.deleteFlag.disable();
        this.searchForm.controls.searchScopeType.disable();
        this.statesStatus.allCompanyFlagEditLock = true;
        break;
      // アップグレード割当:割当先
      case statusAction.LICENSE_UPGRADE_TO_ALLOC:
        this.searchForm.controls.deleteFlag.disable();
        this.searchForm.controls.searchScopeType.disable();
        ...
        break;
      // ライセンス割当
      case statusAction.LICENSE_ALLOC:
      // 割当情報
      case statusAction.LICENSE_REPORT_ALLOC:
        if (status === statusAction.LICENSE_ALLOC) {
          this.searchForm.controls.searchScopeType.disable();
          this.searchForm.controls.searchScopeType.setValue('2');
        }
        this.searchForm.controls.deleteFlag.disable();
        ...
        break;
      // アップグレード情報
      case statusAction.LICENSE_REPORT_UPGRADE:
        this.searchForm.controls.deleteFlag.disable();
        break;
    }
  }
